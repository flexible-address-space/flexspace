...
wormhole_merge_at(struct wormref * const ref, const struct kref * const key)
{
  struct wormleaf * const leaf = wormhole_jump_leaf_write(ref, key);
  struct wormleaf * const next = leaf->next;
  if ((next == NULL) || ((leaf->nr_keys + next->nr_keys) > WH_KPN)) {
    wormleaf_unlock_write(leaf);
    return false;
  }

  wormleaf_lock_write(next, ref);
  return wormhole_meta_leaf_merge(ref, leaf);
}
...
wormhole_split_at(struct wormref * const ref, const struct kref * const key)
{
  ...
  wormleaf_sync_sorted(leaf1);
  const u32 cut = wormleaf_seek(leaf1, key);
  if (cut >= leaf1->nr_keys) {
    wormleaf_unlock_write(leaf1);
    return false;
  }

  const struct kv * const key1 = cut ? wormleaf_kv_at_is(leaf1, cut-1) : leaf1->anchor;
  const struct kv * const key2 = wormleaf_kv_at_is(leaf1, cut);
  if (kv_match(key1, key2)) { // cannot cut if anchor == kv[0]
    wormleaf_unlock_write(leaf1);
    return false;
  }

  struct kv * const anchor2 = wormhole_split_alloc_anchor(key1, key2);
  if (anchor2 == NULL) { // anchor allocation failed
    wormleaf_unlock_write(leaf1);
    return false;
  }

  // create leaf2 with anchor2
  struct wormleaf * const leaf2 = wormleaf_alloc(ref->map, leaf1, leaf1->next, anchor2);
  if (leaf2 == NULL) { // leaf2 allocation failed
    wormhole_free_akey(anchor2);
    wormleaf_unlock_write(leaf1);
    return false;
  }
  ...
  const bool rsm = wormhole_split_meta(ref, leaf2);
  if (unlikely(!rsm)) {
    wormleaf_split_undo(ref->map, leaf1, leaf2, NULL);
    wormleaf_unlock_write(leaf1);
  }
  return rsm;
}
...
// 2-fattest {{{
  static inline bool
wormmeta_tag_check(const struct wormmeta * const meta, const u32 id)
{
  return (meta->l13.v64 & (0x40000lu << id)) != 0lu;
}

  static inline void
wormmeta_tag_set(struct wormmeta * const meta, const u32 id)
{
  // 0 <= id <= 3
  meta->l13.v64 |= (0x40000lu << id);
}

//  static inline void
//wormmeta_tag_unset(struct wormmeta * const meta, const u32 id)
//{
//  // 0 <= id <= 3
//  meta->l13.v64 &= ~(0x40000lu << id);
//}
...
// print-verbose {{{
  static u32
wormmeta_fo(const struct wormmeta * const meta)
{
  const u32 bitmin = wormmeta_bitmin_load(meta);
  const u32 bitmax = wormmeta_bitmax_load(meta);
  if (bitmin == bitmax) {
    return bitmin < WH_FO ? 1 : 0;
  } else {
    return (u32)(__builtin_popcountl(meta->bitmap[0]) + __builtin_popcountl(meta->bitmap[1])
        + __builtin_popcountl(meta->bitmap[2]) + __builtin_popcountl(meta->bitmap[3]));
  }
}
...
{
  (void)pbuf;
  const u32 fo = wormmeta_fo(meta);
  ((u64 *)priv)[fo]++;
}

  static void
wormhole_check_meta_cb_tag0(const struct wormmeta * const meta,
    struct kv * const pbuf, void * const priv)
{
  (void)pbuf;
  const bool tag = wormmeta_tag_check(meta, 0);
  ((u64 *)priv)[tag]++;
}
...
{
  (void)pbuf;
  struct kv * const mkey = wormmeta_keyref_load(meta);
    ...
    wormhole_check_meta_cb cb, void * const priv)
{
  ...
  if (wormmeta_bitmin_load(meta) >= WH_FO)
    return;
...
wormhole_check_meta(struct wormhmap * const hmap, wormhole_check_meta_cb cb, void * const priv)
{
  ...
  static void
wormhole_2fattest(struct wormhole * const map)
{
  struct wormhmap * const hmap = map->hmap;
  struct kv * const pbuf = hmap->pbuf;
  wormhole_prefix(pbuf, 0);
  struct wormmeta * const meta0 = wormhmap_get(hmap, pbuf);
  wormmeta_tag_set(meta0, 0);
  ...
  for (struct wormleaf * leaf = map->leaf0; leaf; leaf = leaf->next) {
    ...
    u32 klen = pbuf->klen;
    while (klen) {
      wormhole_prefix(pbuf, klen);
      struct wormmeta * const meta = wormhmap_get(hmap, pbuf);
      debug_assert(meta);
      wormmeta_tag_set(meta, 0);
      klen &= (klen - 1);
    }
  }
  // count tags
  u64 cnts[2] = {};
  wormhole_check_meta(hmap, wormhole_check_meta_cb_tag0, cnts);
  printf("%s unset %lu set %lu\n", __func__, cnts[0], cnts[1]);
}
...
wormhole_count_leaf(struct wormhole * const map, const bool kv_node)
{
  ...
  for (struct wormleaf * leaf = map->leaf0; leaf; leaf = leaf->next) {
    if (kv_node) { // true: kv size
      for (u32 i = 0; i < leaf->nr_keys; i++)
        x += kv_size(wormleaf_kv_at_is(leaf, i));
    } else { // false: node size (with anchor)
      x += sizeof(*leaf);
      x += key_size(leaf->anchor);
    }
  }
  return x;
}
  ...
  static void
wormhmap_stat(struct wormhmap * const hmap, FILE * const out)
{
  // hmap stat
  const u64 n = ((u64)(hmap->mask)) + 1;
  u64 nu = 0;
  u64 c[WH_BKT_NR+1] = {};
  for (u64 i = 0; i < n; i++) {
    const struct wormmbkt * const slot = &(hmap->pmap[i]);
    u64 nb = 0;
    for (u32 j = 0; j < WH_BKT_NR; j++) {
      if (slot->e[j])
        nb++;
      else
        break;
  ...
  fprintf(out, "MASK %08x SLOTS %lu/%lu CTR 0-8", hmap->mask, nu, n << 3);
...
wormhole_fprint_verbose(struct wormhole * const map, FILE * const out)
{
  ...
  const u32 nr_hmap = map->hmap2[1].pmap ? 2 : 1;
  wormhole_2fattest(map);

  u64 mslab_size[2] = {};
  for (u32 h = 0; h < nr_hmap; h++) {
    u64 fo[WH_FO] = {};
    wormhole_check_meta(&(map->hmap2[h]), wormhole_check_meta_cb_nr, fo);
    u64 tot = 0;
    for (u32 i = 0; i < WH_FO; i += 4) {
      const u64 fo4 = (fo[i] + fo[i+1] + fo[i+2] + fo[i+3]);
      if (fo4) {
        fprintf(out, "[%3u] %8lu [%3u] %8lu [%3u] %8lu [%3u] %8lu\n", i, fo[i], i+1, fo[i+1], i+2, fo[i+2], i+3, fo[i+3]);
        tot += fo4;
      }
    }
    const u64 m1 = fo[0] + fo[1];
    const u64 m2 = tot - m1;
    mslab_size[h] = (m1 * sizeof(struct wormmeta)) + (m2 * (sizeof(struct wormmeta) + sizeof(u64) * 4));
    fprintf(out, "META 32x%lu 64x%lu tot %lu\n", m1, m2, mslab_size[h]);
  }

  const u64 nr_slab_ul = slab_get_nalloc(map->slab_leaf);
  double meta_size2[2] = {};
  for (u32 h = 0; h < nr_hmap; h++) {
    wormhmap_stat(&(map->hmap2[h]), out);
    wormhole_check_meta(&(map->hmap2[h]), wormhole_check_meta_cb_size, &meta_size2[h]);
  }
  fprintf(out, "L-SLAB %lu\n", nr_slab_ul);

  const u64 meta_size = (u64)(meta_size2[0] + meta_size2[1]) + mslab_size[0] + mslab_size[1];
...
wormhole_verify_leaf(struct wormleaf * const leaf)
{
  ...
  if (anchor == NULL) {
    fprintf(stderr, "%s LEAF %p has no anchor\n", __func__, leaf);
    return;
  }
  ...
  u32 i0 = WH_KPN;
  u32 nreh = 0;
  for (u32 i = 0; i < WH_KPN; i++) {
    if (leaf->hs[i].v64 == 0)
      continue;
    struct kv * const kv = wormleaf_kv_at_ih(leaf, i);
    const u64 hash = kv_crc32c_extend(kv_crc32c(kv->kv, kv->klen));
    if (hash != kv->hash) {
      verify_print_twokeys("WRONG HASH", kv, kv);
      return;
    }

    const u16 pkey = wormhole_pkey(kv->hashlo);
    if (pkey != leaf->hs[i].e1) {
      verify_print_twokeys("WRONG PKEY", kv, kv);
      return;
    }

    if (i0 < WH_KPN) {
      if (leaf->hs[i].e1 < leaf->hs[i0].e1) {
        struct kv * const prev = wormleaf_kv_at_ih(leaf, i0);
        verify_print_twokeys("HASH [HASH_LEFT > HASH_RIGHT]", prev, kv);
        return;
      }
      if (leaf->hs[i].v64 == leaf->hs[i0].v64) {
        struct kv * const prev = wormleaf_kv_at_ih(leaf, i0);
        verify_print_twokeys("HASH DUPLICATES", prev, kv);
        return;
      }
    }
    i0 = i;
  ...
  // sorted
  for (u32 i = 1; i < leaf->nr_sorted; i++) {
    struct kv * const left = wormleaf_kv_at_is(leaf, i-1);
    struct kv * const curr = wormleaf_kv_at_is(leaf, i);
    const int cmp = kv_compare(left, curr);
    if (cmp >= 0) {
      verify_print_twokeys("SORTED [KEY_LEFT >= KEY_RIGHT]", left, curr);
      return;
    }
  }

  struct kv * const nanchor = leaf->next ? leaf->next->anchor : NULL;
  // sorted_last < next->anchor
  if (leaf->nr_sorted) {
    struct kv * const first = wormleaf_kv_at_is(leaf, 0);
    const int cmp1 = kv_compare(anchor, first);
    if (cmp1 > 0) {
      verify_print_twokeys("SORTED [ANCHOR >= FIRST_SORTED]", anchor, first);
      return;
    }
    if (nanchor) {
      struct kv * const last = wormleaf_kv_at_is(leaf, leaf->nr_sorted-1);
      const int cmp2 = kv_compare(last, nanchor);
      if (cmp2 >= 0) {
        verify_print_twokeys("SORTED [LAST_SORTED >= NEXT_ANCHOR]", last, nanchor);
        return;
      }
    }
  }

  // unsorted keys between anchors
  for (u32 i = leaf->nr_sorted; i < leaf->nr_keys; i++) {
    struct kv * const k = wormleaf_kv_at_is(leaf, i);
    const int cmp = kv_compare(anchor, k);
    if (cmp > 0) {
      verify_print_twokeys("UNSORTED [ANCHOR > KEY]", anchor, k);
      return;
    }
  }
  ...
  if (nanchor) {
    for (u32 i = leaf->nr_sorted; i < leaf->nr_keys; i++) {
      struct kv * const k = wormleaf_kv_at_is(leaf, i);
      const int cmp1 = kv_compare(k, nanchor);
      if (cmp1 >= 0) {
        verify_print_twokeys("UNSORTED [KEY >= NEXT_ANCHOR]", k, nanchor);
        return;
      }
    }
  }
...
wormhole_verify_leaf_chain(struct wormhole * const map)
{
  ...
  u64 keycount = 0;
  for (struct wormleaf * leaf = map->leaf0; leaf; leaf = leaf->next) {
    wormhole_verify_leaf(leaf);
    leafcount++;
    keycount += leaf->nr_keys;
  }

  fprintf(stderr, "LEAF leafcount %lu keycount %lu\n", leafcount, keycount);
  return leafcount;
}
...
wormhole_verify_meta_tree(struct wormhmap * const hmap, struct kv * const pbuf)
{
  ...
  struct wormmeta * const meta = wormhmap_get(hmap, pbuf);
  if (meta == NULL) {
    fprintf(stderr, "%s NODE NOT FOUND ", __func__);
    kv_print(pbuf, "xn", stderr);
    return 0;
  }

  // test maxplen
  if (pbuf->klen > hmap->maxplen) {
    fprintf(stderr, "%s MAXPLEN too small ", __func__);
    kv_print(pbuf, "xn", stderr);
  }

  // lpath
  struct wormleaf * const lpath = wormmeta_lpath_load(meta);
  struct wormleaf * const lmost = wormmeta_lmost_load(meta);
  const bool lp_ok = (pbuf->klen == lmost->anchor->klen) ? (lpath == lmost) : (lpath == lmost->prev);
  if (!lp_ok) {
    fprintf(stderr, "%s bad lpath %p\n", __func__, lpath);
    kv_print(pbuf, "xn", stderr);
    fprintf(stderr, "LMOST ");
    kv_print(lmost->anchor, "xn", stderr);
    fprintf(stderr, "LMOST->PREV ");
    kv_print(lmost->prev->anchor, "xn", stderr);
    fprintf(stderr, "LMOST->NEXT ");
    kv_print(lmost->next->anchor, "xn", stderr);
  }
  ...
  if (min != wormmeta_bitmin_load(meta))
    fprintf(stderr, "%s min %u != bitmin %u\n", __func__, min, wormmeta_bitmin_load(meta));
  if (max != wormmeta_bitmax_load(meta))
    fprintf(stderr, "%s max %u != bitmax %u\n", __func__, max, wormmeta_bitmax_load(meta));
...
wormhole_verify(struct wormhole * const map)
{
  ...
  fprintf(stderr, "META leafcount %lu %lu\n", c2, c3);
  wormhole_free_mkey(pbuf);
  return (c1 == c2) && (c2 == c3);
}
// }}} verify
    ...
    const char * const opt)
{
    ...
    for (u32 x = 0; x < 2; x++) {
      ...
      for (u64 s = 0; s < nr_slots; s++) {
        ...
        for (u64 i = 0; i < WH_BKT_NR; i++) {
          ...
          struct kv * const keyref = wormmeta_keyref_load(meta);
          wormhole_dump_line(out, keyref, key_size(keyref), "metakey");
        }
      }
    }
  }
  ...
  while (leaf) {
    if (dumpleaf)
      wormhole_dump_line(out, leaf, sizeof(*leaf), "leafnode");
    if (dumpanchor)
      wormhole_dump_line(out, leaf->anchor, kv_size(leaf->anchor), "anchor");
    if (dumpkeys) {
      for (u64 i = 0; i < leaf->nr_keys; i++) {
        struct kv * const kv = wormleaf_kv_at_is(leaf, i);
...
wormhole_print_meta_anchors_rec(struct wormhmap * const hmap, const char * const pattern,
    struct kv * const pbuf, const u32 nseg)
{
  struct wormmeta * const meta = wormhmap_get(hmap, pbuf);
  debug_assert(meta);
  ...
  const u32 plen0 = pbuf->klen;
  const u32 fo = wormmeta_fo(meta);
  const u32 nseg1 = (fo > 1) ? (nseg + 1) : nseg;
  for (u32 i = 0; i < WH_FO; i++) {
    if (wormmeta_bm_test(meta, i)) {
      pbuf->kv[plen0] = (u8)i;
      wormhole_prefix(pbuf, plen0 + 1);
      wormhole_print_meta_anchors_rec(hmap, pattern, pbuf, nseg1);
    }
  }
}
...
wormhole_print_meta_anchors(struct wormhole * const map)
{
  ...
  debug_assert(pbuf);
  kv_dup2(kv_null(), pbuf);
  wormhole_print_meta_anchors_rec(hmap, "xn", pbuf, 0);
  wormhole_free_mkey(pbuf);
}
...
{
  char * const pattern = use_hex ? "xn" : "sn";
  printf("== %s ==\n", __func__);
  ...
  static u64
wormhole_print_meta_lrmost_rec(struct wormhmap * const hmap, const char * const prefix, const char * const pattern,
    struct kv * const pbuf, const u32 depth)
{
  struct wormmeta * const meta = wormhmap_get(hmap, pbuf);
  debug_assert(meta);
  if (wormmeta_bitmin_load(meta) == WH_FO) {
    if (depth)
      fprintf(stdout, "%.*s---* ", (depth << 2) - 3, prefix);
    else
      fprintf(stdout, "LEAF ");
    kv_print(pbuf, pattern, stdout);
    return 1;
  ...
  fprintf(stdout, "%.*s/--" TERMCLR(32) "LM ", depth << 2, prefix);
  kv_print(wormmeta_lmost_load(meta)->anchor, pattern, stdout);
  ...
  if (fo > 1) {
    for (u32 i = 0; i < WH_BMNR; i++) {
      for (u64 bits = meta->bitmap[i]; bits; bits &= (bits - 1lu)) {
        const u32 id = ((u32)__builtin_ctzl(bits)) + (i * 64u);
        pbuf->kv[plen0] = (u8)id;
        wormhole_prefix(pbuf, plen0 + 1);
        nleaf += wormhole_print_meta_lrmost_rec(hmap, prefix, pattern, pbuf, depth + 1);
      }
    }
  } else {
    const u32 id = wormmeta_bitmin_load(meta);
    if (id < WH_FO) {
      pbuf->kv[plen0] = (u8)id;
      wormhole_prefix(pbuf, plen0 + 1);
      nleaf += wormhole_print_meta_lrmost_rec(hmap, prefix, pattern, pbuf, depth + 1);
  ...
  debug_assert(nleaf);
  fprintf(stdout, "%.*s|--" TERMCLR(35) "RM ", depth << 2, prefix);
  kv_print(wormmeta_rmost_load(meta)->anchor, pattern, stdout);
  fprintf(stdout, TERMCLR(0));
  fprintf(stdout, "%.*s" TERMCLR(34) "\\-META %5lu %3u ", depth << 2, prefix, nleaf, fo);
  wormhole_prefix(pbuf, plen0);
  kv_print(pbuf, pattern, stdout);
  fprintf(stdout, TERMCLR(0));
  return nleaf;
}
...
wormhole_print_meta_lrmost(struct wormhole * const map)
{
  ...
  struct wormhmap * const hmap = map->hmap;
  struct kv * const pbuf = wormhole_alloc_mkey(hmap->maxplen + 8);
  const u32 prefixlen = hmap->maxplen * 4;
  char * const prefix = malloc(prefixlen + 16);
  for (u32 i = 0; i < prefixlen; i += 16)
    memcpy(prefix + i, "|   #   !   +   ", 16);
  debug_assert(pbuf);
  kv_dup2(kv_null(), pbuf);
  wormhole_print_meta_lrmost_rec(hmap, prefix, "xn", pbuf, 0);
  wormhole_free_mkey(pbuf);
  free(prefix);
}

  static void
wormhole_print_leaf_helper(struct wormleaf * const leaf, const bool use_hex)
{
  const u32 lcpa = leaf->next ? kv_key_lcp(leaf->anchor, leaf->next->anchor) : 0u;
  char * const pattern = use_hex ? "xn" : "sn";
  printf(TERMCLR(34) "====leaf sorted %u\nanchor: ", leaf->nr_sorted);
  kv_print(leaf->anchor, pattern, stdout);
  printf(TERMCLR(0));
  for (u32 i = 0; i < leaf->nr_sorted; i++) {
    struct kv * const prev = i ? wormleaf_kv_at_is(leaf, i-1) : leaf->anchor;
    struct kv * const curr = wormleaf_kv_at_is(leaf, i);
    const u32 lcp = kv_key_lcp(prev, curr);
    debug_assert(lcp >= lcpa);
    printf("k[%3u]", curr->klen);
    str_print_hex(stdout, curr->kv, lcpa);
    printf(TERMCLR(32));
    str_print_hex(stdout, curr->kv+lcpa, lcp-lcpa);
    printf(TERMCLR(35));
    str_print_hex(stdout, curr->kv+lcp, curr->klen-lcp);
    printf(TERMCLR(0) "\n");
  }
  printf(TERMCLR(35) "----unordered %u\n", leaf->nr_keys - leaf->nr_sorted);
  for (u32 i = leaf->nr_sorted; i < leaf->nr_keys; i++)
    kv_print(wormleaf_kv_at_is(leaf, i), pattern, stdout);
  printf(TERMCLR(0));
}
...
wormhole_print_leaf_keys(struct wormhole * const map, const bool use_hex)
{
  struct wormleaf * leaf = map->leaf0;
  while (leaf) {
    wormhole_print_leaf_helper(leaf, use_hex);
    leaf = leaf->next;
  }
}
// }}} print-more
